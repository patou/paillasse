...
class OrguePalissy(object):
    """
    Orgue dans Palissy.

    p/BIBL,p/PETA,p/IMG,p/REFA,p/PINT,p/WCOM,p/STAT,p/DATE,p/REF,p/DENO,p/ADRS,p/EMPL,p/INTE,p/LIENS,p/PHOTO,p/COM,p/APPL,p/WADRS,p/ETAT,p/DPRO,
    p/VIDEO,p/HIST,p/PROT,p/EDIF,p/COPY,p/TICO,p/PPRO,p/DESC,p/PDEN,p/OBS,p/REFE,p/AUTR,p/DIMS,p/PLOC

    Le champ REFA est utilisé pour faire le lien, si l'édifice hébergeant l'objet est classé, avec la base Mérimée.
    """
    def __init__(self, _liste):
        [self.BIBL,
         ...
         self.DIMS,
         self.PLOC] = _liste
        # WCOM est absent pour quelques enregistrements.
        if self.WCOM == '':
            self.WCOM = self.COM
        return

    def __repr__(self):
        return '<object OrguePalissy {} {} {} {} {} {} {}>'\
            .format(self.WCOM, self.COM, self.EDIF, self.DENO, self.PDEN, self.REF, self.PROT)


class OrguesPalissy(list):
    """
    Base Palissy.
    """
    def __init__(self, fic_csv):
        ...
        :argument: fic_csv : fichier CSV extrait de Palissy
        :return:
        """
        super().__init__()
...
class OrguePalissyPop(object):
    """
    Orgue dans Palissy exporté via POP back-office.
...
if __name__ == "__main__":
    ...
    palissy_pop = OrguesPalissyPop('../97-data/export-pop-palissy.csv')
